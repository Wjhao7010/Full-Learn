...
def get_patch(input_array, i, j, filter_width, filter_height, stride):
    """
    从输入数组中获取本次卷积的区域并且自动适配输入和输入为2D和3D的情况
    """
    start_i = i * stride
    start_j = j * stride
...
def conv(input_array,
         ...
         bias):
    """
    实现2维和3维数组的卷积，conv计算卷积，自动适配2D和3D情况
    """
    chanel_number = input_array.ndim
...
def padding(input_array, zp):
    """
    为数组增加zero padding，自动适配输入2D和3D的情况
    """
    if zp == 0:
        return input_array
    else:
...
class ConvLayer(object):
    ...
    def bp_sensitivity_map(self, sensitivity_array, activator):
        ...
        # full卷积，对sensitivity map进行zero padding （补0）
        # 虽然原始输入的zero padding也能获得残差，但是这个残差不需要向上传递，因此不进行计算
        expanded_width = expanded_array.shape[2]
        zp = (self.input_width +
              self.filter_width - 1 - expanded_width) / 2
        padded_array = padding(expanded_array, zp)
        # 初始化delta_array，用于传递到上一层
        # sensitivity map
    ...
    def expand_sensitivity_map(self, sensitivity_array):
        ...
        # 计算stride为1时候sensitivity map的大小
...
class Filter(object):
    # 滤波器类保存了卷积层的参数以及梯度，使用梯度下降算法来更新参数（权重项，偏置项）
...
class ReluActivator(object):
    # 实现了激活函数，forward为前向计算，backward用来计算导数
    # 该激活函数为Relu函数
    def forward(self, weighted_input):
        # return weighted_input 返回权重输入
